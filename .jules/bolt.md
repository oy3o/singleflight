...
## 2024-05-23 - Memory reduction via return values
**Learning:** Returning values dynamically from deferred blocks (like a boolean flag capturing state inside a lock) saves struct memory by avoiding unnecessary field definitions on heavily instantiated heap structs.
**Action:** In `sync.Pool` reused objects, check if intermediate lock states can be locally cached and returned directly from worker functions rather than persisting them in the struct.
