...
## 2024-06-01 - Avoid Storing Intermediate State as Struct Fields
**Learning:** Avoid storing temporary execution state (e.g., intermediate boolean flags like `shared` or `forgotten`) as fields on structs. Instead, pass state between helper methods and their callers using named return variables. This prevents redundant memory writes and simplifies the struct contract, reducing struct size when padding aligns properly. For instance, removing `shared` and `forgotten` reduced the `call` struct size from 80 bytes to 72 bytes.
**Action:** When evaluating conditions after releasing a mutex, prefer using locally cached variables evaluated under the lock over dereferencing heap-allocated struct fields to prevent data races and reduce memory reads. Returning cached local variables and boolean flags from deferred functions (by modifying named return variables) is an effective and safe way to achieve this. Verify struct size changes with `unsafe.Sizeof`.
