...
## 2024-11-20 - Cache fields under lock to avoid heap dereferences
**Learning:** When evaluating conditions after releasing a mutex in high-performance concurrent Go code, dereferencing heap-allocated struct fields (e.g., `c.shared` or `c.panicErr != nil` after `doCall`) can introduce micro-delays due to memory reads.
**Action:** Return locally cached variables evaluated under the lock from deferred functions (e.g., `(shared bool, panicked bool)`) to prevent extra heap memory reads and data races after the lock is released.
