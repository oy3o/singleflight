...
## 2024-05-02 - Eliminate Struct Field for Local State
**Learning:** Returning values directly from a function (like `doCall` returning `isShared`) instead of storing them in a shared struct (`call.shared`) and reading them back avoids heap memory reads and reduces struct size.
**Action:** Always look for opportunities to replace struct state used locally across function boundaries with direct function return values.
