...
## 2024-05-18 - [Memory Density] Remove temporary execution state from struct
**Learning:** Storing temporary execution state (like the `shared` boolean flag) as fields on heap-allocated structs increases memory footprint and adds an unnecessary memory read. Returning cached local variables and boolean flags from deferred functions using named return variables avoids data races and shrinks struct padding.
**Action:** Prefer using named return variables to pass state between helper methods and their callers instead of storing them as fields on structs in hot paths.
