...
## 2024-05-24 - Pass state via named return variables instead of struct fields
**Learning:** For better cache locality and memory density in hot execution paths, avoid storing temporary execution state (e.g., intermediate boolean flags) as fields on structs. Instead, pass state between helper methods and their callers using named return variables to shrink struct padding and minimize heap footprints.
**Action:** Remove temporary execution state fields from structs and pass them as named return variables to minimize struct size and memory footprints.
