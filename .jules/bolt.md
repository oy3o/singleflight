...
## 2026-04-10 - Avoid redundant zero-initialization for new objects
**Learning:** In Go, `new(T)` allocates zeroed memory by default. Explicitly re-initializing struct fields to zero for a freshly allocated struct is unnecessary CPU work.
**Action:** When using `sync.Pool`, only manually reset fields to zero when the object is successfully retrieved from the pool (e.g., inside an `else` block after checking for `nil`), and skip field resets if `new(T)` was just called.
