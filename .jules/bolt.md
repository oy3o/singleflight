...
## 2024-05-25 - Avoid Struct Fields for Temporary Execution State
**Learning:** For better cache locality and memory density in hot execution paths, avoid storing temporary execution state (e.g., intermediate boolean flags like `shared`) as fields on structs. This bloats the struct size and reduces cache efficiency.
**Action:** Pass temporary execution state between helper methods and their callers using named return variables. This shrinks struct padding, minimizes heap footprint, and avoids unnecessary memory reads.
