...
func (g *Group[K, V]) Do(
	...
	// Capture shared state returned from doCall instead of dereferencing struct field
	// to avoid unnecessary heap memory read and shrink struct padding.
	shared = g.doCall(c, key, fn, ctx)
...
func (g *Group[K, V]) doCall(
	...
	defer func() {
		...
		// 在锁内捕获 shared 状态，通过 named return variable 传递给调用方，
		// 防止 Leader 返回路径无锁读 dups 产生 data race，
		// 避免在 call 结构体上存储一次性的执行状态。
		shared = c.dups > 0
		done := c.done
		g.mu.Unlock()
